import {
  ...
  RangeContext,
  SubstitutionContext
} from './GeneratedAntlr/CalculatorParser';
  ...
  implements CalculatorVisitor<number>
{
  ...
  constructor(
    private substitutionResolver: (substitution: string) => number,
  ...
  visitSubstitution(context: SubstitutionContext): number {
    ...
    this.formulaErrorListener.reportSubstitutionNotFound(
  ...
  visitRange(context: RangeContext): number {
    const resolved = this.rangeResolver({
      start: context.start.text,
      end: context._end.text
    });
    if (resolved != null) {
      return resolved;
    }

    this.formulaErrorListener.reportRangeNotFound(
      context.start.tokenIndex,
      context.text
    );
    return 0;
  ...
  visitMin(context: MinContext): number {
    ...
    if (context._expr.length > 1) {
      for (let i = 1; i < context._expr.length; i++) {
        currentMin = Math.min(
          currentMin,
          this.visitExpression(context._expr[i])
        );
      }
    }
  ...
  visitNegExponent(context: NegExponentContext): number {
    return (
      ...
      Math.pow(10, -1 * this.visitExpression(context.expression(1)))
    );
  }
  ...
  visitArccot(context: ArccotContext): number {
    return (
      Math.PI * 0.5 - Math.atan(this.visitExpression(context.expression()))
    );
  }
